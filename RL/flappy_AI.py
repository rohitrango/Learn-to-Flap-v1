...
try:
    ...
    class QLearningModel:
        ...
        def __init__(self, epsilon=0.35, epsilon_decay=0.9995, learning_rate=0.05, gamma=0.95, maxlen=50000, inp_shape=3, out_shape=2, \
                        batch_size=50, epsilon_min=0.05):
        ...
        def __build_model(self):
            ...
            model = Sequential()
            model.add(Dense(30, input_shape=(self.inp_shape, ), activation='sigmoid'))
            model.add(Dense(30, activation='sigmoid'))
            model.add(Dense(20, activation='sigmoid'))
            model.add(Dense(20, activation='sigmoid'))
            model.add(Dense(self.out_shape))
            model.compile(loss='mse', optimizer=keras.optimizers.adagrad(lr=self.learning_rate), metrics=['accuracy'])
            return model
        ...
        def replay(self):
            ...
            minibatch = random.sample(m, self.batch_size)
            extreme_batch = filter(lambda x: x['reward']!=1, m)
            extreme_batch = random.sample(extreme_batch, min(len(extreme_batch), self.batch_size))
            minibatch = extreme_batch + minibatch
            ...
            for mem in minibatch:
                ...
                if not mem['done']:
                    target = mem['reward'] + self.gamma * np.amax(self.model.predict(mem['next_state'])[0])

                result = self.model.predict(mem['state'])[0]
                result[mem['action']] = target

                self.model.fit(mem['state'], np.array([result]), epochs=1, verbose=False)
                # print(self.model.evaluate(mem['state'], np.array([result])))
            self.max_score = 0
    ...
    def mainGame(movementInfo):
        global clf, model, minVelY, maxVelY, mindely, maxdely, mindelx, maxdelx
        k = PyKeyboard()
        ...
        while True:
            ...
            if state is not None:
                memory = {
                    'state': state, 
                    'action': z,
                    'next_state': np.array([[playerVelY, (delx/GRID_SIZE), (dely/GRID_SIZE)]]),
                    'done': done,
                    'reward': reward,
                }
            ...
            state = np.array([[playerVelY, (delx/GRID_SIZE), (dely/GRID_SIZE)]])
            z = model.act(state)
            print(model.model.predict(state))


            minVelY = min(minVelY, playerVelY)
            maxVelY = max(maxVelY, playerVelY)

            mindely = min(mindely, int(dely/GRID_SIZE))
            maxdely = max(maxdely, int(dely/GRID_SIZE))

            mindelx = min(mindelx, int(delx/GRID_SIZE))
            maxdelx = max(maxdelx, int(delx/GRID_SIZE))

            print(minVelY, maxVelY, mindelx, maxdelx, mindely, maxdely)
